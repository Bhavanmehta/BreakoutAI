...
| **run_scan.py** | The one you run. Discovers the universe, does all of the above for every stock in it, merges `holdings.json` + `sectors.json` if present, computes the Market Mood Index, stores data in a local DuckDB file for research, writes **`../data/breakouts.json`** (the file the website consumes), and updates the track record. |
